program main
  ...
  REAL(kind=dp) :: JVS(LU_NONZERO_2),JVS_orig(LU_NONZERO)
  REAL(kind=dp) :: X_selected(LU_NSEL_2),X_deleted(LU_NDEL_2),X(NVAR)
  REAL(kind=dp) :: JVS1(LU_NONZERO_2),JVS2(LU_NONZERO)
  REAL(kind=dp) :: F(NFIX),RCT(NREACT),Vdot_SEL(LU_NSEL_2),Vdot(NVAR),diff(LU_NSEL_2)
  REAL(kind=dp) :: diff2(LU_NONZERO_2)
  REAL(kind=dp) :: Jcb1(LU_NSEL_2,LU_NSEL_2),Jcb2(NVAR,NVAR),Jcb3(LU_NSEL_2,LU_NSEL_2)
  REAL(kind=dp) :: JacDiff(LU_NSEL_2,LU_NSEL_2)
  INTEGER:: IER,i,j,k
  ...
  DO i=1,LU_NONZERO_2
     Jcb1(LU_IROW_2(i),LU_ICOL_2(i)) = JVS1(i)
  END DO  
  ...
  DO i=1,LU_NSEL_2
    DO j=1,LU_NSEL_2
	  Jcb3(i,j)=Jcb2(select_ind_2(i),select_ind_2(j))
	END DO
  END DO
