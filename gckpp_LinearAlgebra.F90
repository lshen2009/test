...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  USE gckpp_JacobianSP
      ...
      character(len=20)::format_string1,format_string2,format_string3
      OPEN(unit=1101,file='Fun_KppDecomp.txt')

      a = 0. ! mz_rs_20050606
      IER = 0
      DO k=1,NVAR
            CALL num2str(k,name1)
            write (1101,*), "#Loop ",trim(name1)
        IF ( ABS(JVS(LU_DIAG(k))) < TINY(a) ) THEN
            IER = k
            RETURN
        END IF
        DO kk = LU_CROW(k), LU_CROW(k+1)-1
              W(LU_ICOL(kk)) = JVS(kk)
              CALL num2str(LU_ICOL(kk),name1)
              CALL num2str(kk,name2)
              write (1101,*),'#DO 1 :', name2
              write (1101,*), "W(",trim(name1),")=JVS(",trim(name2),")"
        END DO
        DO kk = LU_CROW(k), LU_DIAG(k)-1
            CALL num2str(kk,name2)
            write (1101,*),'#DO 2 :', name2
            j = LU_ICOL(kk)
            a = -W(j)/JVS(LU_DIAG(j))
            CALL num2str(j,name1)
            CALL num2str(LU_DIAG(j),name2)
            write(1101,*),"a=-W(",trim(name1),")/JVS(",trim(name2),")"
            W(j) = -a
            write(1101,*),"W(",trim(name1),")=-a"
            DO jj = LU_DIAG(j)+1, LU_CROW(j+1)-1
               W( LU_ICOL(jj) ) = W( LU_ICOL(jj) ) + a*JVS(jj)
               CALL num2str(LU_ICOL(jj),name1)
               CALL num2str(jj,name2)
               write(1101,*),"W(",trim(name1),")=W(",trim(name1),")+a*JVS(",trim(name2),")"
            END DO
         END DO
         DO kk = LU_CROW(k), LU_CROW(k+1)-1
            CALL num2str(kk,name2)
            write (1101,*),'#DO 3 :', name2
            JVS(kk) = W( LU_ICOL(kk))
            CALL num2str(kk,name1)
            CALL num2str(LU_ICOL(kk),name2)
            write(1101,*),"JVS(",trim(name1),")=W(",trim(name2),")"
         END DO
      END DO
      close(1101)
END SUBROUTINE KppDecomp2

! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SUBROUTINE KppDecomp3( JVS, IER )
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!        Sparse LU factorization
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  USE gckpp_JacobianSP
      ...
      DO i=1,NVAR
         CALL num2str(i,name1)
         longname="X("//trim(name1)//")=X("//trim(name1)//")"
         DO j = LU_CROW(i), LU_DIAG(i)-1 
             X(i) = X(i) - JVS(j)*X(LU_ICOL(j));
             CALL num2str(j,name2)
             CALL num2str(LU_ICOL(j),name3)
             longname=trim(longname)//"-JVS("//trim(name2)//")*X("//trim(name3)//")"
             flag=.true.
         END DO 
         IF (flag) THEN
            write(1102,*),trim(longname)
            flag=.FALSE.
         ENDIF
      END DO
      ...
      DO i=NVAR,1,-1
        ...
        CALL num2str(LU_DIAG(i),name4)
        IF (flag) THEN
        longname="X("//trim(name1)//")="//"("//trim(longname)//")"//"/JVS("//trim(name4)//")"
        ELSE 
        longname="X("//trim(name1)//")="//trim(longname)//"/JVS("//trim(name4)//")"
        ENDIF
        write(1102,*),trim(longname)
...
! X - Vector for variables
  ...
  X(7) = X(7)-JVS(15)*X(6)
  X(8) = X(8)-JVS(18)*X(5)-JVS(19)*X(6)-JVS(20)*X(7)
  X(9) = X(9)-JVS(24)*X(8)
  X(10) = X(10)-JVS(27)*X(5)-JVS(28)*X(8)-JVS(29)*X(9)
  X(10) = X(10)/JVS(30)
  X(9) = (X(9)-JVS(26)*X(10))/(JVS(25))
  X(8) = (X(8)-JVS(22)*X(9)-JVS(23)*X(10))/(JVS(21))
  X(7) = (X(7)-JVS(17)*X(8))/(JVS(16))
  X(6) = (X(6)-JVS(14)*X(8))/(JVS(13))
  X(5) = (X(5)-JVS(12)*X(8))/(JVS(11))
  X(4) = (X(4)-JVS(10)*X(9))/(JVS(9))
  X(3) = (X(3)-JVS(7)*X(7)-JVS(8)*X(8))/(JVS(6))
...
! XX - Vector for output variables
  ...
  XX(3) = X(3)/JVS(6)
  XX(4) = X(4)/JVS(9)
  XX(5) = (X(5)-JVS(2)*XX(1))/(JVS(11))
  XX(6) = X(6)/JVS(13)
  XX(7) = (X(7)-JVS(7)*XX(3))/(JVS(16))
  XX(8) = (X(8)-JVS(3)*XX(1)-JVS(8)*XX(3)-JVS(12)*XX(5)-JVS(14)*XX(6)-JVS(17)*XX(7))/(JVS(21))
  XX(9) = (X(9)-JVS(5)*XX(2)-JVS(10)*XX(4)-JVS(22)*XX(8))/(JVS(25))
  XX(10) = (X(10)-JVS(23)*XX(8)-JVS(26)*XX(9))/(JVS(30))
  XX(10) = XX(10)
  XX(9) = XX(9)-JVS(29)*XX(10)
  XX(8) = XX(8)-JVS(24)*XX(9)-JVS(28)*XX(10)
  XX(7) = XX(7)-JVS(20)*XX(8)
  XX(6) = XX(6)-JVS(15)*XX(7)-JVS(19)*XX(8)
  XX(5) = XX(5)-JVS(18)*XX(8)-JVS(27)*XX(10)
  XX(4) = XX(4)
  XX(3) = XX(3)
  XX(2) = XX(2)
  XX(1) = XX(1)
